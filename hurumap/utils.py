...
def get_table_data(table_name, session, geo_code, geo_level):
    ...
    if table:
        ...
        data = session.query(Base.metadata.tables[table_name]).filter(
            Base.metadata.tables[table_name].c.geo_level == geo_level).filter(
            Base.metadata.tables[table_name].c.geo_code == geo_code).all()
